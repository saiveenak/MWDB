...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
InputPathHist = fullfile(InputDir,'in_file.chst');             
...
InputPathSift = fullfile(InputDir,'in_file.sift');             
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%                          Reading from in_file.sift                               %%%%%%%%%%%%%%% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Newmatrix1Sift = zeros((FrameRange(2)- FrameRange(1)+1),135);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%           Creating aggregate Matrix for Video1                                   %%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
iter = 1;
celliter2 = 1;
Newmatrix2Sift = zeros((InputMatrixSift(countV2Sift,2))*InputMatrixSift(countV2Sift,3),135);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%           Creating final aggregate all Video1                                   %%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
InputPathmvect = fullfile(InputDir,'in_file.mvect');             
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while (Matrix1mvect(it,2)==frame_no_mvect)
    ...
    x=Mat1mvect(cell_it,9)-Mat1mvect(cell_it,7);
    y=Mat1mvect(cell_it,10)-Mat1mvect(cell_it,8);
    if y==0
        angle=0;
    else
        angle=y/x;
    end
    Mat1mvect(cell_it,11)=atand(angle);
    if (it+1 >lengthmvect(1))
            break;
    end
    cell_it=cell_it+1;
...
for eachvideomvect = 1:TotalVideomvect
    ...
    Mat2mvect=zeros(Matrix2mvect(lengthmvect(1),3)*Matrix2mvect(lengthmvect(1),3),11);
    frame_it=1;
    ...
    while (Matrix2mvect(it,2)==frame_no_mvect)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%                  creating aggregate matrix for current video                     %%%%%%%%%%%%%%% 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        x=Mat2mvect(cell_it,9)-Mat2mvect(cell_it,7);
        y=Mat2mvect(cell_it,10)-Mat2mvect(cell_it,8);
        if y==0
            angle=0;
        else
            angle=y/x;
        end
        Mat2mvect(cell_it,11)=atand(angle);
        if (it+1 >lengthmvect(1))
                break;
        end
        cell_it=cell_it+1;
        ...
        for eachcellM1 = 1:Matrix1mvect(lenSubSequencemvect(1),3)
            ...
            for eachcellM2 = 1:lenVideomvect
                if (Matrix2mvect(eachcellM2,3) == eachcellM1)
                    ...
                    sim = 0;
                    for it=5:10
                        sim=sim+abs(Mat1mvect(start+eachcellM1-1,it)-Mat2mvect(eachcellM2,it));
                    end
                    sim=sim+abs(Mat1mvect(start+eachcellM1-1,11)-Mat2mvect(eachcellM2,11))/180;

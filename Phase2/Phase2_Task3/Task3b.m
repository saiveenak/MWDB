...
OutputFile1 = 'in_file_d.spca';                                               
OutputPath1 = fullfile(OutputPath,OutputFile1);

OutputFile2 = 'in_file_d2.spca';
...
siftsize = size(SiftPCAMatrix);
Output1 = fopen(OutputPath1,'w');
for printlen = 1:siftsize(1)
    SiftVectorValues = '';
    for eachvector = 1:(str2double(dim)+3)
        if (isempty(SiftVectorValues))
            SiftVectorValues = strcat(num2str(SiftPCAMatrix(printlen,eachvector)));
        else
            SiftVectorValues = strcat(SiftVectorValues,',',num2str(SiftPCAMatrix(printlen,eachvector)));
        end
    end    
    fprintf(Output1,'%s',SiftVectorValues);
    fprintf(Output1,'\n');
end 
fclose(Output1);
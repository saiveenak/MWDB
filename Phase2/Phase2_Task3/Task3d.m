...
dim = input('Enter Target Dimensionality using K-MEANS:- ','s');                 
...
for printlen = 1:ObjKMatrixSize(1)
    ...
    for eachvector = 1:(str2double(dim)+3)
        if (isempty(HistVectorValues))
            HistVectorValues = strcat(num2str(ObjKMatrix(printlen,eachvector)));
        else
            HistVectorValues = strcat(HistVectorValues,',',num2str(ObjKMatrix(printlen,eachvector)));
        end
    end    

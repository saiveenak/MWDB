...
OutputFile1 = 'in_file_d.skm';                                              
OutputPath1 = fullfile(OutputPath,OutputFile1);

OutputFile2 = 'in_file_d2.skm';
...
for printlen = 1:ObjKMatrixSize(1)
    ...
    for eachvector = 1:(str2double(dim)+3)
        if (isempty(SiftVectorValues))
            SiftVectorValues = strcat(num2str(ObjKMatrix(printlen,eachvector)));
        else
            SiftVectorValues = strcat(SiftVectorValues,',',num2str(ObjKMatrix(printlen,eachvector)));
        end
    end    

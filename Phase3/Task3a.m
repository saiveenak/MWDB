...
%VideoMatrix(1:totalvideos,2) = [1,10,2,3,4,5,6,7,8,9];
...
while ( videocount <= totalvideos )
    ...
    if (videocount == 1)
        ...
        intvalue = CountsMatrix(1:(videocount-1),2);
        %disp(intvalue(1));
        valuesum = sum(intvalue);
        CountsMatrix(videocount,2) = (framecount - valuesum);
        %disp(CountsMatrix(videocount,2));
    end
...
[eigenvectors,eigenvalues] = eig(NewSimMatrix);
...
RankMatrix(1:fframecount,3) = eigenvectors(1:fframecount,1);

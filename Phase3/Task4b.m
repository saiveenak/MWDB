...
CountsMatrix(1:iter,1) = VideoMatrix(1:iter,2);
...
s2 = str2double(s2);
s3 = str2double(s3);
SM = eye(fframecount,fframecount);
sm = [s1;s2;s3];
for seed = 1:3
    for iter = 1:fframecount
        if ( FTempMatrix(iter,1) == sm(1) && FTempMatrix(iter,2) == sm(2) )
            SM((FTempMatrix(iter,3)),(FTempMatrix(iter,3))) = (1.3/0.3);
        end    
    end    
end    
%%%%%%% Seed Nodes matrix %%%%%%%%%%%%
...
%%%%%%%% ASCOS Modified Formula Implementation %%%%%%%%%%%%%%%%%%%%%%
Aprime = SimilarityMatrix';
Aprime = -Aprime;
P = NewSimMatrix;
Adprime = exp(Aprime);
A = ones(fframecount,fframecount) - Adprime;
Q = P.*A;
c = input('Enter the c value(between 0 and 1): ');
Prod1 = (eye(fframecount,fframecount) - (c*(Q')));
Product1 = inv(Prod1);
Product2 = (1-c)*SM;
S = Product1*Product2;
Sprime = sum(S,1);
...
RankMatrix(1:fframecount,3) = Sp(1:fframecount,1);
